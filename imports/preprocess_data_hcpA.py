...
# root_folder = '/data/hzb/project/Brain_Predict_Score/HCPA/data_preprocess_fmri_4/'
root_folder = '/data/hzb/project/Brain_Predict_Score/HCPA/T1w_data_preprocess/'


data_folder = os.path.join(root_folder, 'HCPA_pcp/Gordon/filt_noglobal/')


phenotype = os.path.join(root_folder, 'HCPA_pcp/HCP_Aging_phenotype-select.csv')



# /data/hzb/project/Brain_Predict_Score/HCPA/data_preprocess_fmri_4/HCPA_pcp/Gordon/filt_noglobal
...
def get_timeseries_hcpa_t1(subject_list, atlas_name, silence=False):
    """
        subject_list : list of short subject IDs in string format
        atlas_name   : the atlas based on which the timeseries are generated e.g. aal, cc200
    returns:
    ...
    for i in range(len(subject_list)):
        subject_folder = os.path.join(data_folder, subject_list[i])
        ro_file = [f for f in os.listdir(subject_folder) if f.endswith('_ROI_feature_' + atlas_name + '.txt')]   #HCA9734999_ROI_feature_Gordon333.txt
        fl = os.path.join(subject_folder, ro_file[0])
        if silence != True:
            print("Reading timeseries file %s" % fl)
        timeseries.append(np.loadtxt(fl, skiprows=0).T)
...
def get_subject_score_4fmri(subject_list, score):
    ...
    with open(phenotype) as csv_file:
        reader = csv.DictReader(csv_file)
        for row in reader:
            if row['src_subject_id'] in subject_list2:
                if row[score] == 'M':
                    scores_dict[row['src_subject_id']]=0
                else:
                    scores_dict[row['src_subject_id']]=1      #rsfmrilist
